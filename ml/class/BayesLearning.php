...
class BayesLearning {
  ...
  function registerFilters() {
    if(is_object($this->courpus)) {
      ...
      foreach($categoryList as $eachCategory) {
        ...
        $checkSql = sprintf('SELECT COUNT(*) AS FLAG_EXISTS FROM m_filter WHERE name="%s"', $eachFilterName);
        ...
        if($eachResultSet['FLAG_EXISTS'] == 0) {
          /* フィルタが未登録の場合 */
          $insertSql = sprintf('INSERT INTO m_filter(name, target_corpus_size, exclude_corpus_size, th_score) VALUES("%s", 0, 0, 0)', $eachFilterName);
  ...
  */
  function getFilterIDByCategory($category) {
    if(strlen($category) > 0) {
      ...
      $baseSql = sprintf('SELECT * FROM m_filter WHERE name="%s"', $eachFilterName);
    ...
    if(is_numeric($filterId)) {
      ...
      foreach($wordCountMap as $eachWord => $eachWordCount) {
        ...
        if($eachResultSet['FLAG_EXISTS'] == 0) {
          ...
          $insertSql = sprintf('INSERT INTO t_word_count(filter_id, corpus_type, word, word_count, last_learned_datetime) VALUES(%d, 0, "%s", %d, NOW())', $filterId, $eachWord, $eachWordCount);
          $sth = $this->dbh->prepare($insertSql);
          $sth->execute();
        } else {
          /* 単語がテーブル t_word_count に登録済みの場合 */
          $updateSql = sprintf('UPDATE t_word_count SET word_count=word_count+%d, last_learned_datetime=NOW() WHERE filter_id=%d AND corpus_type=0 AND word="%s"', $eachWordCount, $filterId, $eachWord);
      ...
      $updateSql = sprintf('UPDATE m_filter SET target_corpus_size=target_corpus_size+%d, last_learned_datetime=NOW() WHERE id=%d', sizeof($fileList), $filterId);
      $sth = $this->dbh->prepare($updateSql);
      $sth->execute();
      return true;
    } else {
    ...
    if(is_numeric($filterId)) {
      ...
      foreach($categoryList as $eachExcludeCategory) {
        ...
        if($category !== $eachExcludeCategory) {
          ...
          foreach($wordCountMap as $eachWord => $eachWordCount) {
            /* 既に登録済みの単語かどうか判定する。 */
            $checkSql = sprintf('SELECT COUNT(*) AS FLAG_EXISTS FROM t_word_count WHERE filter_id=%d AND corpus_type=1 AND word="%s"', $filterId, $eachWord);
            $sth = $this->dbh->prepare($checkSql);
            $sth->execute();
            ...
            if($eachResultSet['FLAG_EXISTS'] == 0) {
              /* 単語がテーブル t_word_count に未登録のとき */
              $insertSql = sprintf('INSERT INTO t_word_count(filter_id, corpus_type, word, word_count, last_learned_datetime) VALUES(%d, 1, "%s", %d, NOW())', $filterId, $eachWord, $eachWordCount);
              $sth = $this->dbh->prepare($insertSql);
              $sth->execute();
            } else {
              /* 単語がテーブル t_word_count に登録済みの場合 */
              $updateSql = sprintf('UPDATE t_word_count SET word_count=word_count+%d, last_learned_datetime=NOW() WHERE filter_id=%d AND corpus_type=1 AND word="%s"', $eachWordCount, $filterId, $eachWord);
              $sth = $this->dbh->prepare($updateSql);
              $sth->execute();
            }
          }
  ...
  function getWordCountList($filter_id, $corpus_type) {
    if(is_numeric($filter_id)) {
      if(is_numeric($corpus_type)) {
        ...
        $readingSql = sprintf('SELECT word,word_count FROM t_word_count WHERE filter_id=%d AND corpus_type=%d', $filter_id, $corpus_type);
  ...
  */
  ...
  /** 日本語のテキストを指定し、その判定用スコアを計算する。
  * @param string text 判定用スコアを計算する日本語のテキスト
