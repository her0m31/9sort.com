  ...
  if(is_object($bayesKentei)) {
    ...
    foreach($categoryList as $eachCategory) {
      ...
      */
      ...
      /* 度数分布表を表示する */
      printf("判定用スコアの分布\n");
      printf("判定用スコアの範囲  | %37s | %37s \n", sprintf('%s のコーパス', $eachCategory), sprintf('%s 以外のコーパス', $eachCategory));
      ...
      for($scoreRange = 0; $scoreRange <= 10; $scoreRange++) {
        ...
        if($scoreRange < 10) {
          $eachRange = sprintf('%1.1f <= score <  %1.1f', $scoreRange / 10, $scoreRange / 10 + 0.1);
        } else {
          $eachRange = 'score = 1.0';
        }
        ...
        printf("%19s | %32s | %32s\n", $eachRange, sprintf('%1.3f ％', $eachDistTarget * 100), sprintf('%1.3f ％', $eachDistExclude * 100));
      }
      printf("\n\n");
    }
  } else {
