<?php
/** サンプルプログラム : nlp_ai/training_code/code-5-5-2-e.php
  * [説明]
  * 「5.5.2.評価用データを使って性能を調べよう：20 News Groupsコーパス編」の内容を扱うサンプルプログラム。
  * 評価用データを使い、ベイジアンフィルタの性能を測定する。
  * 
...
if( is_object( $c20ng ) ){
    if( is_object( $bayes20ng ) ){
        ...
        foreach( $categoryList as $eachCategory )
        {
            ...
            /* $eachCategory に含まれる評価用コーパスファイルの一覧を取得する */
            $fileList       = $c20ng->getTestCorpusList( $eachCategory );
            ...
            {
                $corpusBody             = $c20ng->getTestContent( $eachFile );
            ...
            foreach( $categoryList as $eachCategoryExclude )
            {
                ...
                if( $eachCategory !== $eachCategoryExclude ){
                    ...
                    $fileListExclude    = $c20ng->getTestCorpusList( $eachCategoryExclude );
